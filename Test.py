...
class Plateau():
    ...
    def getAdjacentTilesFromTile(self, tile):
        ...
        x = tile[0]
        y = tile[1]
        if abs(x+y+1)<=3 and abs(x)+abs(y+1)<=3 and not(abs(x)>0 and y==-1) and not(x>=3):
            adjTiles.append((x, y+1))
        if abs(x+y-1)<=3 and abs(x)+abs(y-1)<=3 and not(abs(x)>0 and y==-3) and not(x>=3):
            adjTiles.append((x, y-1))
        if abs(x+y+1)<=3 and abs(x+1)+abs(y)<=3 and not(abs(x+1)>0 and y==-2) and not(x+1>=3):
            adjTiles.append((x+1, y))
        if abs(x+y-1)<=3 and abs(x-1)+abs(y)<=3 and not(abs(x-1)>0 and y==-2) and not(x-1>=3):
            adjTiles.append((x-1, y))
        if x % 2 == 0:
            if abs(x+y+2)<=3 and abs(x+1)+abs(y+1)<=3 and not(abs(x+1)>0 and y==-1) and not(x+1>=3):
                adjTiles.append((x+1, y+1))
            if abs(x+y)<=3 and abs(x-1)+abs(y+1)<=3 and not(abs(x-1)>0 and y==-1) and not(x-1>=3):
                adjTiles.append((x-1, y+1))
        else:
            if abs(x+y)<=3 and abs(x+1)+abs(y-1)<=3 and not(abs(x+1)>0 and y==-3) and not(x+1>=3):
                adjTiles.append((x+1, y-1))
            if abs(x+y-2)<=3 and abs(x-1)+abs(y-1)<=3 and not(abs(x-1)>0 and y==-3) and not(x-1>=3):
                adjTiles.append((x-1, y-1))
        return adjTiles
    ...
    def getAdjacentTilesFromVertice(self, vertice):
        ...
        if abs(x+y//2)<=3 and abs(x)+abs(y//2)<=3 and not(abs(x)>0 and y//2==-2) and not(x>=3):
...
print(P.getAdjacentTilesFromVertice((3,1)))
print(P.getAdjacentEdgesFromEdge((6,1)))
"""
print(P.getEdgesFromTile((0,0)))
print(P.getEdgesFromTile((0,-1)))
print(P.getEdgesFromTile((1,0)))
print(P.getEdgesFromTile((1,1)))
print(P.getEdgesFromTile((0,1)))
print(P.getEdgesFromTile((-1,1)))
print(P.getEdgesFromTile((-1,0)))
"""
"""
print(P.getVerticesFromTile((0,0)))
print(P.getEdgesFromTile((0,0)))
print(P.getAdjacentTilesFromVertice((0,0)))
print(P.getAdjacentVerticesFromVertice((0,0)))
print(P.getVerticesOfEdge((0,0)))
print(P.getAdjacentEdgesFromEdge((1,1)))
print(P.getAdjacentEdgesFromVertice((0,0)))
"""